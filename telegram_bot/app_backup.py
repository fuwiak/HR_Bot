...
BOOKING_KEYWORDS = [
    ...
    "анастасия", "новосёлова",  # имя HR-консультанта
...
CHAT_PROMPT = """
Ты дружелюбный помощник HR-отдела. Общайся на "вы", кратко и по делу, используй эмодзи для дружелюбия.
...
- ✅ Вежливо прощайся при прощании: "До свидания! Буду рада помочь снова"
...
async def show_services_page(query: CallbackQuery):
    ...
    try:
        ...
        for i, service in enumerate(page_services, 1):
            ...
            if "консультация" in name.lower():
                emoji = "💬"
            elif "собеседование" in name.lower() or "интервью" in name.lower():
                emoji = "👔"
            elif "онбординг" in name.lower() or "адаптация" in name.lower():
                emoji = "🚀"
            elif "обучение" in name.lower() or "тренинг" in name.lower():
                emoji = "📚"
            elif "оценка" in name.lower() or "аттестация" in name.lower():
                emoji = "📊"
            elif "развитие" in name.lower():
                emoji = "📈"
            else:
                emoji = "✨"
...
async def start_booking_process(query: CallbackQuery):
    ...
    text += "\n👥 *Доступные HR-специалисты:*\n"
    for master in masters[:5]:
        name = master.get('name', 'HR-специалист')
    ...
    text += "Например: `Хочу записаться на консультацию к Анжеле на завтра в 14:00`"
    ...
    if is_booking_result:
        ...
        if parsed_data["has_all_info"]:
            try:
                ...
                answer += f"👤 *HR-специалист:* {parsed_data['master']}\n"
                answer += f"⏰ *Время:* {parsed_data['datetime']}\n\n"
                answer += "Спасибо за запись! Ждем вас в HR-отделе! ✨"
            ...
            # Если пользователь отвечает на вопрос бота (например, просто "Анастасия"), 
            ...
            if mentioned_master:
                ...
                system_msg = """Ты помощник HR-отдела. КРИТИЧЕСКИ ВАЖНО: 
...
- Если видишь блок "НАЙДЕНА УСЛУГА" - используй ТОЧНО эти данные"""
            ...
            is_lead_query = not any(keyword in text_lower for keyword in [
                "запись", "записаться", "записать", "специалист", "консультация", "собеседование", 
                "онбординг", "обучение", "роман", "анжела", "хочу записаться"
            ])
        ...
        if is_general_question:
            ...
            if is_pure_goodbye:
                ...
                if not has_said_goodbye_before:
                    answer = "До свидания! Буду рада помочь снова. 😊"
                else:
                    answer = "До свидания! 😊"
...
- ✅ Если это ПРОЩАНИЕ - вежливо попрощайся: "До свидания! Буду рада помочь снова"
