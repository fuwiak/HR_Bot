...
async def monitor_emails(telegram_bot, interval_minutes: int = 15, require_approval: bool = True):
    ...
    ВАЖНО: Автоматическая отправка ответов ОТКЛЮЧЕНА.
    ...
    Args:
        ...
        interval_minutes: Интервал проверки в минутах
        require_approval: Требовать ли подтверждение консультанта перед отправкой (не используется, ответы отключены)
    """
    if not SCENARIOS_AVAILABLE:
    ...
    log.warning(f"⚠️ [Мониторинг] Автоматическая отправка ответов ОТКЛЮЧЕНА")
    log.info(f"📧 [Мониторинг] Письма будут только отправляться в канал")
    ...
    while True:
        try:
            ...
            for email_data in emails:
                ...
                # Обрабатываем письмо через Сценарий 2 (только отправка в канал, без ответа)
                result = await process_lead_email(
                    email_data=email_data,
                    require_approval=True,  # Всегда требуем подтверждение (но ответы отключены)
                    telegram_bot=telegram_bot
                )
                
                if result.get("success") or result.get("sent_to_channel"):
                    processed_emails.add(email_key)
                    log.info(f"✅ [Мониторинг] Письмо отправлено в канал: {email_key}")
