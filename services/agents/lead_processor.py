...
async def generate_proposal(lead_request: str, lead_contact: Dict, rag_results: Optional[List[Dict]] = None, conversation_history: Optional[str] = None) -> str:
    ...
    try:
        ...
        proposal = await generate_with_fallback(
            ...
            system_content="Ты помощник консультанта для генерации коммерческих предложений. Пиши деловым, но дружелюбным стилем. НИКОГДА не используй Markdown форматирование (звездочки *, заголовки #, ### и т.д.) - пиши обычным текстом без форматирования.",
            max_tokens=3000,
            temperature=0.7
        )
