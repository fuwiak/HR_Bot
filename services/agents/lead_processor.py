...
- НИКОГДА не используй множественное число при упоминании консультанта (не "мы", "наша команда", "наши специалисты")
...
- ❌ НИКОГДА не используй Markdown форматирование (звездочки *, подчеркивания __, заголовки #, ### и т.д.)
- ❌ НИКОГДА не используй заголовки типа "### Призыв к действию" или "## Этапы работ"
- ❌ НИКОГДА не используй символы * для выделения текста
- ❌ НИКОГДА не используй символы # для заголовков
- ✅ Пиши обычным текстом, без форматирования
- ✅ Используй пустые строки между абзацами для структуры
- ✅ Если нужно выделить разделы - используй просто текст, например "Этапы работ:" без символов

СТРУКТУРА КП (пиши естественным текстом, без заголовков и форматирования):
...
7. Заключение с предложением следующих шагов (БЕЗ фразы "Призыв к действию")
...
async def generate_proposal(lead_request: str, lead_contact: Dict, rag_results: Optional[List[Dict]] = None, conversation_history: Optional[str] = None) -> str:
    ...
    try:
        ...
        proposal = await generate_with_fallback(
            ...
            system_content="Ты помощник консультанта для генерации коммерческих предложений. Пиши деловым, но дружелюбным стилем. КРИТИЧЕСКИ ВАЖНО: Работает ОДИН консультант - Анастасия Новосёлова. Используй единственное число (я, мне, я проведу), НИКОГДА не используй множественное число (мы, наша команда, наши специалисты). НИКОГДА не используй Markdown форматирование (звездочки *, заголовки #, ### и т.д.) - пиши обычным текстом без форматирования.",
            max_tokens=3000,
            temperature=0.7
        )
