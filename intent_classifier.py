...
def classify_intent_with_llm(text: str, openrouter_api_key: str = None, openrouter_url: str = None) -> Tuple[float, Dict]:
    ...
    try:
        ...
        classification_prompt = f"""Ты помощник HR-отдела. Определи, является ли следующее сообщение запросом на запись к HR-специалисту.
...
- 0.5-0.9 = вероятно запрос на запись (например: "когда свободно", "нужна консультация")
